...
namespace TgBot.Commands
{
    public static class GetLast
    {
        ...
        public static List<FixtureResponse> lastmatches = new List<FixtureResponse>();
        ...
        private static async void GetQueryParams(object sender, MessageEventArgs e)
        {
            ...
            var fixtures = await ApiRepository.GetFixtures($"{ApiClient.Base}/Fixture/teamLast?team={team}");           

            if (fixtures == null)
            {
                await Program.client.SendTextMessageAsync(e.Message.From.Id, @$"<b>Something went wrong!</b>
I don't know team '<i>{team}</i>'.Check if you entered the team name correctly and send the last command again", parseMode: ParseMode.Html);
                Program.client.OnMessage -= GetQueryParams;
...
";
                ...
                InlineKeyboardButton button = new InlineKeyboardButton() { CallbackData = "A" + (lastmatches.Count - 1).ToString(), Text = "⭐️" };
        ...
        public static async void ButtonPressed(object sender, CallbackQueryEventArgs e)
            ...
            try
            {
                ...
                if (e.CallbackQuery.Data.StartsWith("A"))
                {
                    ...
                    var toAdd = lastmatches[int.Parse(buttonText)];
